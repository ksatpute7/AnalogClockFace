...
function table_creation() {
    ...
    for (i=0; i<10; i++) {
        table_inner = "<tr>"
        for (j=0; j<10; j++) {
            table_inner += "<td id='r" + i + "c" + j +"' class=\"table_data\"'><span>" +(((i*10)+j+1)+(startingValue-1))+"</span></td>"
        }
        table_inner += "</tr>"
...
function applyColor(cell_obj)
    {
	var selectedCell = ($(cell_obj).find("span").text()-startingValue);
