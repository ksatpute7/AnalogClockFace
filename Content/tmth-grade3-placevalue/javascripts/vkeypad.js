...
window.vKeyPad = function(TextBoxSelector, ParentContainerId, Options){
    ...
    this.onFocusTxtBoxBg = this.Options.onFocusTxtBoxBg ? this.Options.onFocusTxtBoxBg : "#faecc9";
...
vKeyPad.prototype = {
	...
	if (!isTextBox && !isKeyPad && this.isBlurOnWindowClick){
	...
	if (this.isFocusOnClick) {
		...
		if (vK.CurrentFocus) {
		    ...
		    this.checkBtnState();
		    this.onTextBoxFocus($(vK.CurrentFocus));
		}
	    }
	}
    },
    ...
    blur:function(){
	var vK = this.getSelf();
	if (this.textBoxOrgBg && vK.CurrentFocus) $(vK.CurrentFocus).css({"background": this.textBoxOrgBg});
	if (vK.CurrentFocus){
	    ...
	    this.onTextBoxBlur($(vK.CurrentFocus));
	}
	vK.CurrentFocus = null;
    },
